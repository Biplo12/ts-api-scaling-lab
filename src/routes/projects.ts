...
import { idParams, projectShape, taskInListShape } from './schemas.js';
...
const listResponse = {
  200: {
    ...
    properties: {
      ...
      tasks: { type: 'array', items: taskInListShape },
    },
  },
} as const;
...
export default async function projectRoutes(app: FastifyInstance): Promise<void> {
  app.get<{ Params: { id: number } }>(
    '/projects/:id/tasks',
    { schema: { params: idParams, response: listResponse } },
    async (request, reply) => {
      ...
      const tasks = rows.map((r) => ({
        ...r,
        commentCount: countByTask.get(r.id) ?? 0,
      }));
